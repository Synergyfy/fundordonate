...
use GrowfundPro\Mails\CampaignHalfMilestoneReachedMail;
use Growfund\QueryBuilder;

class CampaignHalfMilestoneReachedSchedule implements RecurrableScheduler
{
    ...
    public function handle()
    {
        foreach ($this->users as $user) {
            growfund_scheduler()->resolve(CampaignHalfMilestoneReachedMail::class)
                ->with([
					'receiver_user_id' => $user->id,
					'campaign_id' => $this->campaign_id
				])
                ->group('growfund_campaign_half_milestone_mails')
                ->schedule_email();
        }
    }
