...
class CampaignHalfMilestoneReachedMail extends Mailer
{
    protected $content_key = MailKeys::DONOR_CAMPAIGN_HALF_MILESTONE_ACHIEVED;
    ...
    public function with($data)
    {
        ...
        if (! growfund_app()->is_donation_mode()) {
            $this->using(MailKeys::BACKER_CAMPAIGN_HALF_FUNDED);
        }

        $this->set_receiver_user_id($user->get_id());
        $this->to($user->get_email());

        $campaign_dto = (new CampaignService())->get_by_id($data['campaign_id']);
            ...
            array_merge(
                ...
                [
                    ...
                    'campaign_link_button' => growfund_renderer()->get_html('mails.components.link-button', [
                        'text' => __('Donate Now', 'growfund'),
                        'link' => $campaign['campaign_url'],
                        'colors' => $this->get_colors(),
                    ]),
                ],
                growfund_app()->is_donation_mode()
                    ? ['donor_name' => $user->get_display_name()]
                    : ['backer_name' => $user->get_display_name()]
            )
        );
    }
}
