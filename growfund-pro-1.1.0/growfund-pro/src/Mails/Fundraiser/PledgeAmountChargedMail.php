...
class PledgeAmountChargedMail extends Mailer
{
    protected $content_key = MailKeys::FUNDRAISER_PLEDGE_AMOUNT_CHARGED;

    public function with($data)
    {

        if (!isset($data['pledge_id'])) {
            throw new InvalidArgumentException(esc_html__('Pledge ID is required', 'growfund-pro'));
        }
        ...
        if ($pledge_dto->status !== PledgeStatus::BACKED) {
            $this->ignore_mail();
        }
        ...
        return parent::with([
            'charged_pledge_summary' => growfund_renderer()->get_html('mails.components.fundraiser.charged-pledge-summary', [
                'pledge' => $pledge
            ]),
