...
export function SocialShare({ title, slug, shortDescription }: Props) {
  const [open, setOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);

  const url = typeof window !== "undefined" ? `${window.location.origin}/campaigns/${slug}` : "";
  const shareText = `${title} - ${shortDescription || "Check out this campaign!"}`;
  ...
  const handleNativeShare = async () => {
    try {
      ...
      // User cancelled or share failed — open dropdown fallback
      setOpen(true);
    }
  };
  ...
  const socialLinks = [
    ...
    },
    {
      ...
      shareUrl: `mailto:?subject=${encodedText}&body=Check%20out%20this%20campaign:%20${encodedUrl}`,
    },
  ];
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {open && (
        <div
          className="absolute bottom-full left-0 z-50 mb-2 w-64 rounded-xl border border-gray-200 bg-white p-3 shadow-lg"
          role="menu"
          ...
          <div className="flex items-center gap-2 px-1">
            {socialLinks.map((platform) => (
              <a
                ...
                role="menuitem"
                className={`flex h-8 w-8 items-center justify-center rounded-full transition-colors ${platform.color}`}
