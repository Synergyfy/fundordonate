...
const NAV_ITEMS = [
  { label: "Overview", path: "/fundraiser", icon: "📊" },
  { label: "My Campaigns", path: "/fundraiser/campaigns", icon: "🎯" },
  { label: "Earnings", path: "/fundraiser/earnings", icon: "💰" },
  { label: "Withdrawals", path: "/fundraiser/withdrawals", icon: "🏦" },
];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="flex h-14 items-center border-b border-gray-200 px-4">
          <Link to="/fundraiser" className="text-lg font-bold text-primary-600">FundorDonate</Link>
          <span className="ml-2 rounded bg-blue-100 px-1.5 py-0.5 text-xs font-medium text-blue-700">Fundraiser</span>
        </div>

        <nav className="mt-2 px-2.5">
          {NAV_ITEMS.map((item) => {
            const isActive = location.pathname === item.path || (item.path !== "/fundraiser" && location.pathname.startsWith(item.path));
            return (
              <Link
                key={item.path}
                to={item.path}
